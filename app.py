...
def file_upload(dataset: str):
    ...
    if request.method == "POST":
        ...
        if file:
            ...
            )  # Append the timestamp to avoid conflicts in names
            if (
                "fiq-category" in request.form
            ):  # if the user upload an image to FashionIQ it must specify the category
            ...
            return redirect(
                url_for("relative_caption", dataset=dataset, reference_name=filename)
            )
...
@app.route(
    "/<string:dataset>/<string:reference_name>/<string:old_caption>", methods=["POST"]
)
@app.route("/<string:dataset>/<string:reference_name>", methods=["POST"])
def custom_caption(
    dataset: str, reference_name: str, old_caption: Optional[str] = None
):
    """
    ...
    return redirect(
        url_for(
            "results", dataset=dataset, reference_name=reference_name, caption=caption
        )
    )
...
def results(dataset: str, reference_name: str, caption: str):
    ...
    if dataset == "fashionIQ":
        ...
        sorted_index_names, target_name = compute_fashionIQ_results(
            caption, combiner, n_retrieved, reference_name
        )
...
def compute_fashionIQ_results(
    caption: str, combiner: Combiner, n_retrieved: int, reference_name: str
) -> Tuple[np.array, str]:
    """
    ...
    try:
        ...
        Exception
    ):  # raise an exception if the reference image has been uploaded by the user
        image_path = (
            app.config["UPLOAD_FOLDER"] / "fashionIQ" / dress_type / reference_name
        )
        pil_image = PIL.Image.open(image_path).convert("RGB")
        image = targetpad_transform(1.25, clip_model.visual.input_resolution)(
            pil_image
        ).to(device)
    ...
    with torch.no_grad():
        ...
        predicted_features = combiner.combine_features(
            reference_features, text_features
        ).squeeze(0)
...
def get_image(image_name: str, dim: Optional[int] = None, gt: Optional[str] = None):
    ...
    if image_name in fashion_index_names:
        image_path = (
            server_base_path / "fashionIQ_dataset" / "images" / f"{image_name}.jpg"
        )
    else:  # Search for an uploaded image
        for iter_path in app.config["UPLOAD_FOLDER"].rglob("*"):
...
def _load_assets():
    ...
    fashionIQ_combiner = torch.hub.load(
        server_base_path, source="local", model="combiner", dataset="fashionIQ"
    )
    fashionIQ_combiner = (
        torch.jit.script(fashionIQ_combiner).type(data_type).to(device).eval()
    )
...
def load_fashionIQ_assets():
    ...
    for dress_type in ["dress", "shirt"]:
        with open(
            server_base_path
            / "fashionIQ_dataset"
            / "captions"
            / f"cap.{dress_type}.val.json"
        ) as f:
            dress_type_captions = json.load(f)
            captions = [
                dict(caption, dress_type=f"{dress_type}")
                for caption in dress_type_captions
            ]
            fashionIQ_val_triplets.extend(captions)

    global fashionIQ_val_dress_index_features
    fashionIQ_val_dress_index_features = (
        torch.load(
            data_path / "fashionIQ_val_dress_index_features.pt", map_location=device
        )
        .type(data_type)
        .cpu()
    )
    ...
    fashionIQ_test_dress_index_features = (
        torch.load(
            data_path / "fashionIQ_test_dress_index_features.pt", map_location=device
        )
        .type(data_type)
        .cpu()
    )
    ...
    fashionIQ_dress_index_names = (
        fashionIQ_val_dress_index_names + fashionIQ_test_dress_index_names
    )

    global fashionIQ_val_shirt_index_features
    fashionIQ_val_shirt_index_features = (
        torch.load(
            data_path / "fashionIQ_val_shirt_index_features.pt", map_location=device
        )
        .type(data_type)
        .cpu()
    )
    ...
    fashionIQ_test_shirt_index_features = (
        torch.load(
            data_path / "fashionIQ_test_shirt_index_features.pt", map_location=device
        )
        .type(data_type)
        .cpu()
    )
    ...
    fashionIQ_shirt_index_names = (
        fashionIQ_val_shirt_index_names + fashionIQ_test_shirt_index_names
    )

    global fashion_index_features
    global fashion_index_names
    fashion_index_features = torch.vstack(
        (fashionIQ_dress_index_features, fashionIQ_shirt_index_features)
    )
    fashion_index_names = fashionIQ_dress_index_names + fashionIQ_shirt_index_names
